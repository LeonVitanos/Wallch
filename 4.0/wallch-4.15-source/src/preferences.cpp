    ...
    ui(new Ui::preferences)
{
    ...
    */
    ...
    {
        ui->startupCheckBox->setEnabled(false);
        ui->label_18->setEnabled(false);
        ui->onceCheckBox->setEnabled(false);
        ui->label_6->setEnabled(false);
        ui->startup_timeout_spinbox->setEnabled(false);
        ui->label_7->setEnabled(false);
        ui->label->setEnabled(false);
        ui->exeNotFound_label->setText(tr("wallch.exe could not be found. Start-up options cannot be enabled."));
    }
#endif
    ...
    historyWasEnabledAtFirst_=ui->history_checkbox->isChecked();
#ifdef ON_LINUX
    keybinder_init();
#endif
...
void Preferences::on_saveButton_clicked()
{
    ...
    */
    ...
    if(!gv.saveHistory && historyWasEnabledAtFirst_){
        //History logging was disabled, if there are logs, ask to delete them
        QSettings historySettings(HISTORY_SETTINGS);
        if(historySettings.allKeys().count()!=0){
            if(QMessageBox::question(this, tr("History"), tr("You just disabled the History logging. Do you want to delete the existent logs?"))==QMessageBox::Yes){
    ...
    {
        if(customIntervalsInSeconds_temp.count()>0)
        {
            ...
            for (short i = 0; i < listCount; ++i) {
                settings->setArrayIndex(i);
                settings->setValue("item", customIntervalsInSeconds_temp.at(i));
            }
            settings->endArray();
        }
        else
        {
            gv.customIntervalsInSeconds=customIntervalsInSeconds_temp=DEFAULT_INTERVALS_IN_SECONDS;
            settings->beginWriteArray("custom_intervals_in_seconds");
...
void Preferences::on_reset_clicked()
{
    if (QMessageBox::question(this, tr("Reset Preferences"), tr("Are you sure you want to reset your Wallch\npreferences?")) == QMessageBox::Yes)
    {
        ...
        customIntervalsInSeconds_temp.clear();
...
void Preferences::on_add_custom_interval_clicked()
{
    ...
    for (short i=customIntervalsInSeconds_temp.count()-1; i>0; i--)
    {
        ...
        else if(customIntervalsInSeconds_temp[i]==customIntervalsInSeconds_temp[i-1])
        {
            ...
            QMessageBox::warning(this, tr("Error"), tr("Interval")+" \""+ui->time_spinBox->text()+" "+timeType+"\" "+tr("already exists."));
            break;
        }
        else
        {
            customIntervalsChanged_=true;
            ui->listWidget->insertItem(i, ui->time_spinBox->text()+" "+timeType);
            break;
        }
        if(i==1)
        {
            customIntervalsChanged_=true;
            ui->listWidget->insertItem(0, ui->time_spinBox->text()+" "+timeType);
        }
    }
}
