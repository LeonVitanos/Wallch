...
MainWindow::MainWindow(Global *globalParser, WebsiteSnapshot *websiteSnapshot, WallpaperManager *wallpaperManager, int timeout_count, int last_random_delay, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MainWindow)
{
    ...
    connect(globalParser_, SIGNAL(averageColorChanged()), this, SLOT(setButtonColor()));
    ...
    //Ubuntu applications do not have the menubar placed at the window, so we need to resize the height of the window
    if(ui->menuBar->isHidden())
    {
        this->resize(this->width(), this->height()-27);
    }
    ...
    if(gv.randomTimeFrom>gv.randomTimeTo-3){
        globalParser_->error("The randomness is misconfigured, please head to the Preferences dialog to configure it properly!");
...
void MainWindow::setupKeyboardShortcuts(){
    ...
    //menu shortcuts
    (void) new QShortcut(Qt::CTRL + Qt::Key_Q, this, SLOT(on_actionQuit_Ctrl_Q_triggered()));
    (void) new QShortcut(Qt::CTRL + Qt::Key_H, this, SLOT(on_actionHistory_triggered()));
    (void) new QShortcut(Qt::CTRL + Qt::Key_P, this, SLOT(on_action_Preferences_triggered()));
...
void MainWindow::imageTransition(const QString &filename)
{
    ...
    if(filename=="preview_clock"){
        QImage *image = wallpaperClocksPreview(gv.defaultWallpaperClock, ui->minutes_checkBox->isChecked(),
    ...
    {
        setupAnimationsAndChangeImage(scaleWallpapersPreview(filename));
    }
}
...
void MainWindow::setPreviewImage(QImage *image){
    ...
    if(image && !image->isNull()){
        ...
        ColoringType coloringType=globalParser_->getColoringType();
...
void MainWindow::setStyle(){
    ...
    if(gv.currentDE==UnityGnome || gv.currentDE==Gnome || gv.currentDE==Mate){
        ...
        QString style;
        if(gv.currentDE==Mate){
            style=globalParser_->gsettingsGet("org.mate.background", "picture-options");
        }
        else
        {
            style=globalParser_->gsettingsGet("org.gnome.desktop.background", "picture-options");
        }
        short index=0;
        if (style=="zoom")
        {
...
void MainWindow::setButtonColor()
{
    QString colorName;
#ifdef ON_LINUX
    colorName=globalParser_->getPrimaryColor();
#else
...
void MainWindow::on_image_style_combo_currentIndexChanged(int index)
{
    ...
    if(gv.currentDE==Gnome || gv.currentDE==UnityGnome || gv.currentDE==Mate){
        ...
        if(gv.currentDE==Mate){
            globalParser_->gsettingsSet("org.mate.background", "picture-options", type);
        }
        else
        {
...
void MainWindow::bindKey(const QString &key){
    if(!keybinder_bind(key.toLocal8Bit().data(), nextKeySignal, NULL)){
        ...
        globalParser_->error("I probably could not bind the key sequence: '"+key+"'");
    }
}
...
void MainWindow::changeCurrentThemeTo(const QString &theme)
{
    ...
    else
    {
        ...
        if(gv.currentDE==Gnome || gv.currentDE==UnityGnome){
            curTheme = globalParser_->gsettingsGet("org.gnome.desktop.interface", "gtk-theme");
        }
        else
        {
...
void MainWindow::indicatorSetNormal(){
    globalParser_->changeIndicatorIcon("normal");
}
...
#ifdef ON_LINUX
        globalParser_->changeIndicatorSelection("none");
#endif //#ifdef ON_LINUX
...
void MainWindow::startPauseWallpaperChangingProcess(){
    ...
    if (actAsStart_){
            ...
            if(shuffleWasChecked_){
                if(firstRandomImageIsntRandom_){
                    ...
                    if(ui->listWidget->currentItem()->isSelected()){
                        wallpaperManager_->setRandomMode(true, -1, ui->listWidget->currentRow());
                    }
                }
                else
                {
        ...
        globalParser_->changeIndicatorSelection("wallpapers");
#else
    ...
    else
    {
        ...
        ui->startButton->setIcon(QIcon::fromTheme("media-playback-start", QIcon(":/icons/Pictures/media-playback-start.svg")));
#ifdef ON_LINUX
        globalParser_->changeIndicatorSelection("pause");
        if(gv.currentDE==UnityGnome){
            dbusmenu_menuitem_property_set_bool(gv.unityPauseAction, DBUSMENU_MENUITEM_PROP_VISIBLE, false);
        }
#else
...
void MainWindow::on_stopButton_clicked(){
    ...
    startButtonsSetEnabled(ui->listWidget->count() >= LEAST_WALLPAPERS_FOR_START);
...
void MainWindow::on_next_Button_clicked()
{
    ...
    updateSecondsTimer_->stop();
#ifdef ON_LINUX
    globalParser_->changeIndicatorIcon("right");
...
#ifdef ON_LINUX
    globalParser_->changeIndicatorIcon("left");
    indicatorChangeNormalTimer_->start(INDICATOR_SCROLL_INTERVAL);
#endif
    globalParser_->resetSleepProtection(secondsLeft_);
...
bool MainWindow::currentFolderExists()
{
    ...
    else
    {
        ...
        ui->pictures_location_comboBox->setItemText(index, globalParser_->basenameOf(currentFolder_)+" (0)" );
        ui->listWidget->clear();
...
void MainWindow::on_listWidget_itemDoubleClicked()
{
    ...
    wallpaperManager_->addToPreviousWallpapers(pic);
    wallpaperManager_->setBackground(pic, true, true, 1);
    if(gv.setAverageColor){
        setButtonColor();
    }
    if(gv.rotateImages && gv.iconMode){
...
void MainWindow::removeImageFromDisk(){
    ...
    {
        QString imageFilename = getPathOfListItem(ui->listWidget->currentRow());
        ...
        else
        {
            ...
            QString cache_filename=gv.cachePath+globalParser_->originalToCacheName(imageFilename);
            ...
            if(QFile::remove(cache_filename)){
                ...
                globalParser_->addToCacheSize(cache_file_size*-1);
            }
        }
    }
    else
...
void MainWindow::removeImagesFromDisk(){
    ...
    if(QMessageBox::question(this, tr("Warning!"), tr("Image deletion.")+"<br><br>"+tr("This action is going to permanently delete")+" "+QString::number(selectedCount)+" "+tr("images. Are you sure?"))==QMessageBox::Yes){
        ...
        for(int i=0;i<selectedCount;i++){
            ...
            {
                //remove the cache of the image, in case it exists
                QString cacheFilename=gv.cachePath+globalParser_->originalToCacheName(curImage);
                qint64 cacheFileSize=QFile(cacheFilename).size();
                if(QFile::remove(cacheFilename)){
                    //file successfully removed, remove the size of it from the current cache size!
                    globalParser_->addToCacheSize(cacheFileSize*-1);
                }
            }
        }
...
void MainWindow::on_listWidget_customContextMenuRequested()
{
    ...
    if (selectedCount<2){
        ...
        connect(enterAction, SIGNAL(triggered()), this, SLOT(on_listWidget_itemDoubleClicked()));
...
void MainWindow::addFolderForMonitor(const QString &folder){
    if(QDir(folder).exists()){
        ...
        {
            if(globalParser_->foldersAreSame(folder, ui->pictures_location_comboBox->itemData(i).toString()))
            {
...
void MainWindow::searchFor(const QString &term){
    ...
    if(gv.iconMode){
        for(int i=0;i<listCount;i++){
            if(ui->listWidget->item(i)->statusTip().isEmpty()){
                searchList_ << globalParser_->basenameOf(ui->listWidget->item(i)->toolTip());
            }
            else
            {
                searchList_ << globalParser_->basenameOf(ui->listWidget->item(i)->statusTip());
            }
        }
    }
    else
    {
        for(int i=0; i<listCount; i++){
            searchList_ << globalParser_->basenameOf(ui->listWidget->item(i)->text());
        }
    }
...
void MainWindow::monitor(const QStringList &paths){
    ...
    case 1:
        ...
        break;
    default:
        ui->pictures_location_comboBox->setItemText(ui->pictures_location_comboBox->currentIndex(), globalParser_->basenameOf(currentFolder_)+" ("+QString::number(itemCount)+ ")");
        break;
    }
}
...
void MainWindow::addFilesToWallpapers(const QString &path, int &itemCount){
    ...
    if(gv.iconMode){
        Q_FOREACH(QString file, currrentDirectory.entryList(IMAGE_FILTERS)){
            ...
            wallpaperManager_->addWallpaper(path+"/"+file);
...
void MainWindow::changeIconsToPaths(){
    ...
    for(int i=0;i<file_count;i++){
        ...
        else
        {
            ...
            ui->listWidget->item(i)->setToolTip("");
        }
...
bool MainWindow::updateIconOf(int index){
    ...
    QString cachedImagePath=globalParser_->originalToCacheName(originalImagePath);
    ...
    else
    {
        ...
        ui->listWidget->item(index)->setToolTip("");
...
void MainWindow::forceUpdateIconOf(int index){
    if(ui->listWidget->item(index)->statusTip().isEmpty()){
        ...
        QString cache_name = gv.cachePath+globalParser_->originalToCacheName(ui->listWidget->item(index)->toolTip());
    ...
    QString cache_full_path=gv.cachePath+globalParser_->originalToCacheName(original_name);
...
void MainWindow::cacheSizeChanged(const QString &newCacheImage){
    if(globalParser_->addToCacheSize(QFile(newCacheImage).size())){
...
void MainWindow::refreshIntervals()
{
    ...
    else
    {
        ...
        for(short i=0; i<size; i++)
        {
            ...
            customIntervals_ << globalParser_->secondsToMinutesHoursDays(seconds);
        }
        settings->endArray();
    }
...
void MainWindow::startLivearth()
{
    ...
    globalParser_->updateStartup();
#ifdef ON_LINUX
    globalParser_->changeIndicatorSelection("earth");
    if(gv.currentDE==UnityGnome){
...
void MainWindow::startPotd(bool launchNow){
    ...
    globalParser_->changeIndicatorSelection("potd");
#else
    Q_EMIT signalRecreateTray();
#endif //#ifdef ON_LINUX
    startUpdateSeconds();
...
{
    globalParser_->openUrl("http://en.wikipedia.org/wiki/Wikipedia:Picture_of_the_day");
}
...
void MainWindow::on_clocksTableWidget_currentCellChanged(int currentRow, int currentColumn, int previousRow, int previousColumn)
{
    ...
    if(gv.wallpaperClocksRunning){
        globalParser_->wallpaperClockNow(gv.defaultWallpaperClock, ui->minutes_checkBox->isChecked(), ui->hour_checkBox->isChecked(), ui->am_checkBox->isChecked(),
                                  ui->day_week_checkBox->isChecked(), ui->day_month_checkBox->isChecked(), ui->month_checkBox->isChecked());
    }
...
void MainWindow::on_activate_clock_clicked()
{
    ...
    if(!ui->activate_clock->isEnabled()){
        ...
        globalParser_->error("No default wallpaper clock has been set. Make sure that after installing the wallpaper clock you have clicked at set as default.");
        return;
    }
    ...
    if(ui->minutes_checkBox->isChecked() || ui->hour_checkBox->isChecked() || (ui->am_checkBox->isChecked() && gv.amPmEnabled) ||
            ui->day_week_checkBox->isChecked() || ui->day_month_checkBox->isChecked() || ui->month_checkBox->isChecked())
    {
        globalParser_->readClockIni(gv.defaultWallpaperClock);
        QString currentWallpaperClock = globalParser_->wallpaperClockNow(gv.defaultWallpaperClock, ui->minutes_checkBox->isChecked(), ui->hour_checkBox->isChecked(), ui->am_checkBox->isChecked(),
                                                                  ui->day_week_checkBox->isChecked(), ui->day_month_checkBox->isChecked(), ui->month_checkBox->isChecked());
        if(gv.setAverageColor){
            setAverageColor(currentWallpaperClock);
        ...
        globalParser_->changeIndicatorSelection("clocks");
#else
        Q_EMIT signalRecreateTray();
#endif //#ifdef ON_LINUX
...
void MainWindow::calculateSecondsLeftForWallClocks(){
    ...
    else if(ui->am_checkBox->isChecked() && gv.amPmEnabled)
    {
        ...
        if (timeNow.hour()>=12){
            seconds_left_for_am_pm=timeNow.secsTo(QTime(23,59,59))+2;
        }
        else{
            seconds_left_for_am_pm=timeNow.secsTo(QTime(12,0,1));
        }
...
void MainWindow::on_activate_website_clicked()
{
    ...
    globalParser_->changeIndicatorSelection("website");
#else
    Q_EMIT signalRecreateTray();
#endif //#ifdef ON_LINUX
    setProgressbarsValue(100);
...
void MainWindow::loadWallpapersPage(){
    ...
    ui->listWidget->setItemDelegate(new HideGrayLinesDelegate(ui->listWidget));
...
void MainWindow::on_potd_viewer_Button_clicked()
{
    ...
    if(!Global(true).connectedToInternet()){
        QMessageBox::information(this, tr("Error!"), tr("You are not connected to internet."));
        return;
    }
...
{
    ui->donateButton->setIcon(QIcon(":/icons/Pictures/btn_donate_LG_pressed.gif"));
}

void MainWindow::on_donateButton_released()
{
    ui->donateButton->setIcon(QIcon(":/icons/Pictures/btn_donate_LG.gif"));
}
